...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--partition', default='trainval')
    #parser.add_argument('--partitions', nargs='+', default=['train2015', 'test2015'], type=str)
    parser.add_argument('--num_workers', default=2, type=int)
    parser.add_argument('--data_root', default='/root/whzh/codes/HOI/upt/vcoco')
...
class Engine(pocket.core.DistributedLearningEngine):
    ...
    def eval(self, postprocessors, thresh=0.1):
        ...
        for image, target in self._train_loader:
...
class VCOCOObject(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = torch.cat([torch.ones_like(target['objects']), target['objects']])
        ...
        # if(len(target['labels'])==0 or int(target['labels'][-1])==-1):
        #     return image, None
        return image, target
...
def initialise(args):
    ...
    if os.path.exists(args.pretrained) and not args.eval:
    ...
    if args.partition.startswith('train'):
    ...
    if args.partition.startswith('test'):
        transforms = T.Compose([
    ...
    im_dir = {'trainval': 'train2014', 'test': 'val2014'}
    dataset = VCOCOObject(VCOCO(
        root=os.path.join(args.data_root, f"mscoco2014/{im_dir[args.partition]}"),
        anno_file=os.path.join(args.data_root, f"instances_vcoco_{args.partition}.json"),
...
def main(rank, args):
    ...
    #if args.eval:
    args.partition='test'
    _, _, _, test_dataset = initialise(args)
    test_ampler = torch.utils.data.SequentialSampler(test_dataset)
    test_dataloader = DataLoader(
        dataset, sampler=test_ampler,
        batch_size=1, collate_fn=collate_fn,
        num_workers=args.num_workers,
        drop_last=False
    )    
    engine._train_loader = test_dataloader
    print("start eval")
    ap, rec = engine.eval(postprocessors)
    print(f"The mAP is {ap.mean().item():.4f}, the mRec is {rec.mean().item():.4f}")
