...
class VCOCOeval(object):
  ...
  def __init__(self, vsrl_annot_file, coco_annot_file, 
    ...
    self.image_ids = np.sort(np.loadtxt(open(split_file, 'r'), dtype = np.int))
    # simple check
  ...
  def _do_eval(self, detections_file, mode='scenario_1', ovr_thresh=0.5, ignore_point=True):
    start_time = time.time()    
    vcocodb = self._get_vcocodb()
    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
    print('_get_vcocodb time {}'.format(total_time_str)) 
    
    if mode=='agent':
      start_time = time.time()
      self._do_agent_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, ignore_point=ignore_point)
      total_time = time.time() - start_time
      total_time_str = str(datetime.timedelta(seconds=int(total_time)))
      print('_do_agent_eval time {}'.format(total_time_str))
    else:
      start_time = time.time()
      self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type=mode, ignore_point= ignore_point)
      total_time = time.time() - start_time
      total_time_str = str(datetime.timedelta(seconds=int(total_time)))
      print('_do_role_eval {} time {}'.format(mode, total_time_str))


  
  def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1', ignore_point=True):

    with open(detections_file, 'rb') as f:
      dets = pickle.load(f)
    print("pred query num: " +str(len(dets)))
    print("GT anno num: " +str(len(vcocodb)))
    ...
    roles_no_agent = [ len(r)>=2 for r in self.roles]
    roles_no_agent = list(np.array(range(self.num_actions))[roles_no_agent])
    
    imgId_dets = dict()
    for det in dets:
      if det['image_id'] in imgId_dets.keys():
        imgId_dets[det['image_id']].append(det)
      else: 
        imgId_dets[det['image_id']] = list()   
        imgId_dets[det['image_id']].append(det)
    ...
    for i in range(len(vcocodb)):
      ...
      #TODO: have problem      
      if image_id in imgId_dets.keys():
        pred_agents, pred_roles = self._collect_detections_for_image(imgId_dets[image_id])
      else: 
        continue
      ...
      for aid in roles_no_agent:
        ...
        for rid in range(len(self.roles[aid])-1): 
          ...
          valid = np.where( ~((np.isnan(agent_scores) == True) + (agent_scores==0)) )[0]
    ...
    role_ap[:] = np.nan
    recall = np.zeros((self.num_actions, 2), dtype=np.float32)
    recall[:] = np.nan
    for aid in roles_no_agent:
      # if len(self.roles[aid])<2:
      #   continue
      if 'point' in self.actions[aid] and ignore_point:
        continue
      ...
      for rid in range(len(self.roles[aid])-1):
        ...
        recall[aid, rid] = np.max(rec)
        
    map = np.nanmean(role_ap) 
    m_recall = np.nanmean(recall) 
    print('---------Reporting Role AP (%)------------------')
    for aid in range(self.num_actions):
      if len(self.roles[aid])<2: continue
      for rid in range(len(self.roles[aid])-1):
        print('{: >23}: AP = {:0.2f}  RECALL = {:0.2f} (#pos = {:d})'.format(
          self.actions[aid]+'-'+self.roles[aid][rid+1], role_ap[aid, rid]*100.0, recall[aid, rid] *100.0, int(npos[aid])))
    print('Average Role [%s] AP = %.2f  recall=  %.2f'%(eval_type, map * 100.00,  m_recall * 100.00))  
    print('---------------------------------------------') 



  def _do_agent_eval(self, vcocodb, detections_file, ovr_thresh=0.5, ignore_point=True):
    ...
    for i in range(len(vcocodb)):
      ...
      #TODO: have problem 
      # # It can be seen that there is a code bug here that causes both det and gt to loop
      if image_id not in imgId_dets.keys():
        continue      
      pred_agents, _ = self._collect_detections_for_image(imgId_dets[image_id])
    ...
    agent_ap[:] = np.nan
    recall = np.zeros((self.num_actions), dtype=np.float32)
    recall[:] = np.nan
    for aid in range(self.num_actions):
      if 'point' in self.actions[aid] and ignore_point:
        continue
      ...
      recall[aid] = np.max(rec)
    map = np.nanmean(agent_ap) 
    m_recall = np.nanmean(recall) 
    print('---------Reporting Agent AP (%)------------------')
    for aid in range(self.num_actions):
      print('{: >20}: AP = {:0.2f} RECALL = {:0.2f} (#pos = {:d})'.format(self.actions[aid], agent_ap[aid]*100.0, recall[aid] *100.0, int(npos[aid])))
    print('Average Agent AP = %.2f  recall=  %.2f'%(map * 100.00, m_recall * 100.00))
    print('---------------------------------------------')
