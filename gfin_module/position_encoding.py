...
def build_1d_position_encoding(representation_size):
    '''
        进行位置编码
    '''
    N_steps = representation_size 
    position_embedding = PositionalEmbedding(N_steps)
...
def build_2d_position_encoding(hidden_dim, position_embedding='sine'):
    '''
        进行位置编码
    '''
...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, x):                                                        #   图像，形状为[batch_size x 3 x H x W]
        h, w = x.shape[-2:]             
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x, mask):                                                #   batched图像，形状为[batch_size x 3 x H x W]（实际上未使用）                                               #   [batch_size x H x W]的二进制掩码（单通道黑白），在填充像素为1
